...
#
# ATLAS o-band grid.  Pipeline = run_grid.py, parameters = config_o.py.
...
# Stages A+B (ATLAS12 models + SYNTHE) under MPI
/opt/ohpc/pub/mpi/openmpi3-gnu8/4.0.2/bin/mpirun -n ${SLURM_NTASKS:-20} \
    --mca pml ob1 --mca btl tcp,self \
    $PY src/run_grid.py $CONFIG

# Stage C (assemble) serially, NOT under mpirun — writes the grid durably.
